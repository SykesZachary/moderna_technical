...
hcmv_egfp_aaseq = data_extraction(hcmv_egfp, 'c5mky7_hcmv_egfp_aa.fasta')  # Amino acid sequence for hcmv_
terc_201_seq = data_extraction(terc_201, 'Homo_sapiens_TERC_201_sequence.fasta')

# Reverse translating C5MKY7 based on codon usage frequencies
hcmv_aaseq_sm = SeqManipulation(hcmv_egfp_aaseq[0])  # Class initialized with amino acid sequence
rt_hcmv_egfp = hcmv_aaseq_sm.reverse_translate(hsap_codon_table)  # Reverse translation occurs
file_output(
    hcmv_egfp,
    'C5MKY7_HCMV_EGFP_ReverseTranslate_CodonFrequency.fasta',
    f'{hcmv_egfp_aaseq[1]} | Potential RNA Sequence | Reverse Translated with Codon Frequency',
    rt_hcmv_egfp,
)  # RNA Sequence for C5MKY7 output

# RNA synthesis of TERC-201 cDNA
terc_201_comp = terc_201_seq[0].complement()  # Taking the complement of the cDNA
terc_201_rna = terc_201_comp.transcribe()  # Transcribing the complemented strand
file_output(
    terc_201,
    'Homo_sapiens_TERC_201_rna_transcript.fasta',
    f'{terc_201_seq} | TERC-201 | RNA sequence for cDNA:lncRNA',
    terc_201_rna,
